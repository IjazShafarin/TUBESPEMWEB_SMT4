...
if (isset($_POST['simpan'])){
    ...
    if (mysqli_num_rows($cekUsername)) {
        echo "<script>
                alert('username sudah terpakai, user baru gagal di 
                registrasi ! ');
    ...
    if ($password !== $password2) {
        echo "<script>
                alert('konfirmasi password tidak sesuai , user baru 
                gagal di registrasi ! ');
                window.location = 'tambah-user.php';
                </script>";
                return;
...
if (isset($_POST['update'])){
    ...
    if($username !== $userLama){
        if (mysqli_num_rows($cekUsername)) {
            echo "<script>
                alert('username sudah terpakai, data user gagal di 
                perbarui ! ');
                window.location = 'index.php';
                </script>";
                return;
...
if (isset($_POST['ganti-password'])) {
    ...
    if ($newPass !== $confPass) {
        echo "<script>
            alert('Password gagal diperbarui, Konfirmasi 
            password tidak sama.. ');
